...
function upgradeNetwork() {
  docker inspect -f '{{.Config.Volumes}}' orderer.log-network | grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.log-network
  docker cp -a orderer.log-network:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.log-network
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.log-network

  for PEER in peer0.org1.log-network peer1.org1.log-network peer2.org1.log-network peer3.Org1.log-network; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.log-network/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml docker-compose-cas.yaml 
  cd crypto-config/peerOrganizations/org2.log-network/ca/
  PRIV_KEY=$(ls *_sk)
  #cd "$CURRENT_DIR"
...
# (``peer0.org1.log-network`` & ``peer0.org2.log-network``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile OSLogChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile OSLogChannel -outputAnchorPeersUpdate ./channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
